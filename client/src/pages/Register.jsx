...
import { Hexagon, Lock, Mail, User, Github, Loader2 } from "lucide-react";
...
export default function Register({ onRegisterSuccess }) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      toast.success("Demo Account Registered successfully!");
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in duration-500">
        <div className="flex flex-col items-center mb-8 text-center space-y-2">
          <div className="w-16 h-16 bg-primary/10 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(168,85,247,0.2)] border border-primary/20 mb-4 backdrop-blur-md">
            <Hexagon size={32} className="text-secondary animate-[pulse_4s_ease-in-out_infinite]" />
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight text-foreground text-glow text-gradient">Join FileShare Pro</h1>
          <p className="text-muted-foreground font-medium text-sm max-w-sm">
            Create your workspace and start sharing files seamlessly today.
          </p>
        </div>
        ...
        <Card className="glass-card border-none shadow-2xl backdrop-blur-2xl bg-background/60 dark:bg-black/40">
          <CardHeader className="space-y-1 pb-6">
            ...
            <CardDescription className="text-center text-sm">
              Enter your details (or mock data) to get started instantly.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-background/80 dark:bg-[#09090b]/80 px-4 text-muted-foreground font-semibold backdrop-blur-md rounded-full">Or register with</span>
              </div>
            </div>
          ...
          <CardFooter className="flex justify-center pb-8">
            <p className="text-sm text-muted-foreground font-medium">
              Already have an account? <Link to="/login" className="text-secondary hover:text-secondary/80 hover:underline transition-colors">Sign in</Link>
            </p>
          </CardFooter>
        </Card>
      </div>
